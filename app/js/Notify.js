;(function($, exports, d, util) {
    ...
    Notify.prototype.warn = function(msg, isAuto) {
        var warn = this.items.warn ? this.items.warn : createElement('warn'),
            $msg = warn.$msg;
        this.items.warn = warn;
        $msg.text(msg);
        ...
        if (isAuto)
            setTimeout(function() {
                warn.$off.trigger('click');
            }, Notify.DURATION);
    }
    Notify.prototype.success = function(msg, isAuto) {
        var success = this.items.success ? this.items.success : createElement('success'),
            $msg = success.$msg;
        this.items.success = success;
        $msg.text(msg);
        ...
        if (isAuto)
            setTimeout(function() {
                success.$off.trigger('click');
            }, Notify.DURATION);
    }
    Notify.prototype.error = function(msg, isAuto) {
        var error = this.items.error ? this.items.error : createElement('error'),
            $msg = error.$msg;
        ...
        if (isAuto)
            setTimeout(function() {
                error.$off.trigger('click');
            }, Notify.DURATION);
    }

    function createElement(klass) {
        var $ele = $(d.createElement('div'));
        $ele.addClass('notify-new-element off').addClass(klass);

        var $msg = $(d.createElement('div'));
        $msg.addClass('notify-new-element-msg');

        var $off = $(d.createElement('div'));
        $off.addClass('notify-new-element-off')
        .on('click', function(e) {
            $ele.removeClass('on').addClass('off');
            e.stopPropagation();
        });
        $ele.append($off).append($msg);
        $('body').append($ele);
        return {$ele:$ele, $msg: $msg, $off: $off,
        show: function() {
...
})(jQuery, window, document, window.util);